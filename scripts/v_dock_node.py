...
class vDockNode:
    def __init__(self):
        ...
        self.target_length = rospy.get_param('~target_length', 0.45) # Find the lines with a length closest to BC (50cm)
        ...
        self.V_shape_angle = rospy.get_param('~V_shape_angle', [70, 105])
        self.min_num_beams = rospy.get_param('~min_num_beams', 15) # how many consecutive point should i see before i consider it a line
        self.min_acceptable_detection_dist = rospy.get_param('~min_acceptable_detection_dist', 3.5) # tf is only published if distance is less than this value.
...
# -----------------------------------------
    ...
    def laser_callback(self, scan):
        ...
        if (time.time() - self.time_prev_call) >= 1.5:
            ...
            y_filtered = medfilt(y, kernel_size=7)  # Adjust kernel_size as needed
            y = y_filtered
            ...
            if len(lines) >= 2:
                ...
                for i in range(len(v_lines) - 1):
                    ...
                    if gap_between_lines <= self.max_gap_threshold:  
                        ...
                        # if self.V_shape_angle[0] <= angle_degrees <= self.V_shape_angle[1]: # if 100 <= angle_degrees <= 120: # if 70 <= angle_degrees <= 105:
                        if 0 <= angle_degrees <= 255:
                            ...
                            print("using relative odom distance_to_detection: ", distance_to_detection)
                            ...
                            if self.map_frame != None:
                                if self.noamclposebefore == True:
                                    ...
                                    print("using amcl distance_to_detection: ", distance_to_detection)
